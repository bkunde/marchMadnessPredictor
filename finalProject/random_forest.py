...
class DataFrameSelector(sklearn.base.BaseEstimator, sklearn.base.TransformerMixin):
    ...
    def __init__(self, do_predictors=True, do_numerical=True):
        ...
        self.mCategoricalPredictors = ["CONF", "SEED"]
        self.mNumericalPredictors = ["W", "G","ADJOE", "ADJDE", "BARTHAG", "EFG_O", "EFG_D", "TOR", "TORD", "ORB", "DRB", "FTR", "FTRD", "2P_O", "2P_D", "3P_O", "3P_D", "ADJ_T", "WAB"]
...
def make_random_forest_fit_pipeline(my_args):
    ...
    if my_args.print_preprocessed_data:
        items.append(("printer", Printer("Final Preprocessing")))
    dtargs = {
        "n_estimators": my_args.n_estimators,
        "criterion": my_args.criterion,
        "max_depth": my_args.max_depth,
        "min_samples_split": my_args.min_samples_split,
        "min_samples_leaf": my_args.min_samples_leaf,
    ...
    # criterion="entropy", max_depth=2
    items.append(("model", sklearn.ensemble.RandomForestClassifier(**dtargs)))
    return sklearn.pipeline.Pipeline(items)
...
def make_random_forest_params_grid(my_args):
    params = make_predictor_params(my_args)
    tree_params = {
        #"n_estimators": [10],
        "model__criterion": [ "entropy" ], # [ "entropy", "gini" ],
...
def make_random_forest_params_grid_distribution(my_args):
    params = make_predictor_params_distribution(my_args)
    tree_params = {
...
def do_fit(my_args):
    ...
    if my_args.classifier == 'DT':
        pipeline = make_decision_tree_fit_pipeline(my_args)
    elif my_args.classifier == 'random-forest':
...
def do_grid_search(my_args):
    ...
    if my_args.classifier == 'random-forest':
        pipeline = make_random_forest_fit_pipeline(my_args)
        if my_args.big_params == 1:
            fit_params = make_random_forest_params_grid_big(my_args)
        elif my_args.big_params == 2:
            fit_params = make_random_forest_params_grid_distribution(my_args)
        else:
            fit_params = make_random_forest_params_grid(my_args)
...
def do_predict(my_args):
    prediction_file = my_args.prediction_file
    if not os.path.exists(prediction_file):
        raise Exception("preiction_file: {} does not exist.".format(prediction_file))
    
    model_file = my_args.model_file

    X, y= load_data(my_args, prediction_file)
    pipeline = joblib.load(model_file)
    
    basename = get_basename(prediction_file)
    ...
    round_predictions = {}
    rounds = ['R64', 'R32', 'S16', 'E8', 'F4', 'CHAMPIONSHIP', 'WINNER']
    for round_name in rounds:
        round_predictions[round_name] = enforce_round_constraints(predictions, round_name)


    #print data
    print()
    print("{}: prediction: ".format(basename))
    print()
    for round_name, teams in round_predictions.items():
        print(f"Predictions for {round_name}:")
        for team, prediction in zip(team_names, teams):
            print(f"\t{team}")
    """
    for i in range(len(predictions)):
        if predictions[i] != 'NO POSTSEASON':
            count += 1
            print(X['TEAM'][i],": ", predictions[i])
    """
    return

def enforce_round_constraints(predictions, round_name):
    round_constraints = {
        'R64': 64,
        'R32': 32,
        'S16': 16,
    ...
    max_teams = round_constraints[round_name]
    adjusted_predictions = predictions[:max_teams]
...
def parse_args(argv):
    ...
    parser.add_argument('action', default='DT',
                        choices=[ "fit", "score", "show-model", "cross-validate", "grid-search", "show-best-params", "random-search", "predict"], 
                        nargs='?', help="desired action")
    parser.add_argument('--classifier', default='DT',   choices=( "DT", "random-forest",), help="Type of classifier to use")
    ...
    parser.add_argument('--prediction-file',     '-P', default="",    type=str,   help="name of file to use for prediction)")
    ...
    #
    parser.add_argument('--n_estimators', default=10,           type=int,      help="the number of trees in the forest")
